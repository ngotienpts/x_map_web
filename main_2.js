...
if (marker) {
  // Nếu có marker thì tách chuỗi
  [lat, lon] = marker.split(",").map(Number);
} else {
  // Nếu không có thì lấy từ CONFIG mặc định
  lat = 20.978009;
  lon = 105.8121463;
}
...
class MarkerHandler {
    ...
    async initialize() {
        // Check URL for marker parameter
        const params = new URLSearchParams(window.location.search);
        const marker = params.get("marker");
        ...
        if (marker) {
            ...
            if (lat && lon) {
                ...
                await this.handleMarkerLocation(lat, lon);
            }
        }
    }

    async handleMarkerLocation(lat, lon) {
        try {
            // 1. Move map to marker location
            this.moveMapToMarker(lat, lon);
            
            // 2. Create visual marker on map
            this.createVisualMarker(lat, lon);
            
            // 3. Update rotate button to point to marker
            this.updateRotateButton(lat, lon);
            
            // 4. Get and display place name
            await this.getPlaceName(lat, lon);
            
        } catch (error) {
            console.error('Error handling marker location:', error);
        }
    }
    ...
    moveMapToMarker(lat, lon) {
        if (map) {
            ...
            console.log(`Map moved to marker: ${lat}, ${lon}`);
        }
    }

    createVisualMarker(lat, lon) {
        // Create marker element
        this.markerElement = document.createElement('div');
        this.markerElement.className = 'marker-pin';
        this.markerElement.innerHTML = `
            <div class="marker-pin-inner">
                <svg width="32" height="40" viewBox="0 0 32 40" fill="none" xmlns="http://www.w3.org/2000/svg">
                    <path d="M16 0C7.16 0 0 7.16 0 16C0 28 16 40 16 40S32 28 32 16C32 7.16 24.84 0 16 0ZM16 22C12.69 22 10 19.31 10 16C10 12.69 12.69 10 16 10C19.31 10 22 12.69 22 16C22 19.31 19.31 22 16 22Z" fill="#FF4444"/>
        ...
        // Add marker to map
        if (map) {
            new maplibregl.Marker({ element: this.markerElement })
                .setLngLat([lon, lat])
                .addTo(map);
        }
    }

    updateRotateButton(lat, lon) {
        const rotateButton = document.getElementById('rotate-button');
        if (rotateButton && State.user.lat && State.user.lon) {
            // Calculate bearing from user position to marker
            const bearing = utils.calculateBearing(
                State.user.lat, State.user.lon,
                lat, lon
            );
            
            // Update rotate button to point to marker
            rotateButton.style.setProperty('--rotate', `${bearing}deg`);
            rotateButton.title = `Hướng đến: ${this.placeName || 'Vị trí đã chọn'}`;
            
            // Add click handler to rotate button
            rotateButton.addEventListener('click', () => {
                this.rotateMapToMarker(bearing);
            });
            
            console.log(`Rotate button updated to point at ${bearing.toFixed(0)}°`);
        }
    }

    rotateMapToMarker(bearing) {
        if (map) {
            map.easeTo({
                bearing: bearing,
                duration: 1000
    ...
    async getPlaceName(lat, lon) {
        try {
            ...
            this.placeName = this.extractPlaceName(data);
            ...
            this.placeName = 'Vị trí không xác định';
            this.displayPlaceName();
        }
    }

    extractPlaceName(geocodeData) {
        ...
        const nameFields = [
            ...
            address?.amenity,           // Amenities (shops, restaurants, etc.)
            address?.leisure,           // Parks, recreational areas
            address?.building,          // Buildings
            address?.shop,              // Shops
            address?.office,            // Offices
            address?.historic,          // Historic places
            address?.natural,           // Natural features
            address?.landuse,           // Land use areas
            address?.highway,           // Roads (as fallback)
        ...
        return 'Vị trí không rõ';
    }

    displayPlaceName() {
        // Remove existing place name display
        const existing = document.getElementById('marker-place-name');
        if (existing) {
            existing.remove();
        }
        
        // Create new place name display
        const placeNameEl = document.createElement('div');
        placeNameEl.id = 'marker-place-name';
        placeNameEl.className = 'marker-place-name';
        placeNameEl.textContent = this.placeName;
        
        document.body.appendChild(placeNameEl);
        
        // Auto-hide after 5 seconds
        setTimeout(() => {
            if (placeNameEl.parentNode) {
                placeNameEl.style.opacity = '0';
                placeNameEl.style.transition = 'opacity 0.5s ease-out';
                setTimeout(() => {
                    if (placeNameEl.parentNode) {
                        placeNameEl.remove();
                    }
        ...
        console.log(`Place name displayed: ${this.placeName}`);
    }
    ...
    clearMarker() {
        if (this.markerElement && this.markerElement.parentNode) {
            this.markerElement.parentNode.remove();
        }
        
        const placeNameEl = document.getElementById('marker-place-name');
        if (placeNameEl) {
            placeNameEl.remove();
        }
        ...
        const rotateButton = document.getElementById('rotate-button');
        if (rotateButton) {
            rotateButton.style.removeProperty('--rotate');
            rotateButton.title = 'Xoay về Bắc';
        }
        
        // Remove marker from URL
        const url = new URL(window.location);
...
// ===== GLOBAL INSTANCES (ADD MARKER HANDLER) =====
// let gpsTracker, utils, navigationManager, searchManager, uiManager, voiceAudio, markerHandler;
...
async function initializeApp() {
    ...
    try {
        ...
        markerHandler = new MarkerHandler(); // Add marker handler
        ...
        // Initialize marker handler after map is ready
        map.on('load', async () => {
            await markerHandler.initialize();
        });
